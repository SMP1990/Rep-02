    ...
    <div class="flex items-center justify-between h-16 gap-6">
      ...
      <form action="/electronics-store/shop.php" method="get" class="hidden lg:flex flex-1 max-w-md">
        <div class="relative w-full">
          <input type="text" name="q" value="<?= e($_GET['q'] ?? '') ?>" placeholder="Search products, brands..."
      ...
      <div class="flex items-center gap-4 text-slate-200 shrink-0">
        <a href="/electronics-store/cart.php" title="Cart" class="relative hover:text-brand-light">
          🛒
          <span class="absolute -top-2 -right-2 bg-accent text-ink text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center"><?= $cart_count ?></span>
        </a>
